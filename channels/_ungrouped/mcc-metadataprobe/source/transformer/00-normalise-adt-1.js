// ADT normalisation for Epic (step 1)
        var segments = msg['PID'];
        if (segments != undefined) {
          var mrn = '';
          for each (var cx in msg['PID']['PID.3']) {
            if (cx['PID.3.4']['PID.3.4.1'].toString() == 'EPIC') {
        ...
        var routing = {
          'MAIN': { queue: 'main.adt', priority: 1, ack: true },
          'NORTH': { queue: 'north.adt', priority: 2, ack: true },
          'SOUTH': { queue: 'south.adt', priority: 2, ack: false },
          'CLINIC': { queue: 'clinic.adt', priority: 3, ack: false }
        };
        var site = msg['MSH']['MSH.4']['MSH.4.1'].toString();
        var route = routing[site];
        if (route == undefined) {
          route = { queue: 'unrouted.adt', priority: 9, ack: false };
          logger.warn('unrouted ADT from ' + site);
        }
        channelMap.put('route_1', route.queue);
        channelMap.put('priority_1', route.priority);

        // Timestamp normalisation: Epic sends local time without an offset.
